...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $password_ver) {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (email, phone, name, birth, password, address, role) VALUES (?, ?, ?, ?, ?, ?, 'user')");
            $stmt->bind_param("ssssss",$email, $phone, $name, $birth, $hashed, $address);
            if ($stmt->execute()) {
